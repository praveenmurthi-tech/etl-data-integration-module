...
from src.models.audit_models import EtlRun, EtlRunStep, Base, FileExtractionLog
...
def log_extracted_file(
    ...
    """Insert a file extraction audit record."""
    with Session(engine) as sess:
        file_log = FileExtractionLog(
            run_id=run_id,
            dataset=dataset,
            file_path=file_path,
            rows=rows,
            ...
            chunk_no=chunk_no,
            status=status,                # can be "success" or "failed"
            ingestion_status="pending",   # 👈 always pending until ingestion step updates it
